...
io.on('connection',(socket) => {
    ...
    socket.on('sendLocation', (coords,callback) => {
        const user = getUser(socket.id)
        socket.emit('locationMessage',generateLocationMessages(user.username,`https://google.com/maps?q=${coords.latitude},${coords.latitude}`))       
        socket.broadcast.to(user.room).emit('locationMessagetoOthers',generateLocationMessages(user.username,`https://google.com/maps?q=${coords.latitude},${coords.latitude}`))
        callback()
    })
})
